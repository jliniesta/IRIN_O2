...
void CTestNeuronExp::RandomPositionAndOrientation ( void )
{
	...
	//PARA GENERAR LA LUZ AMARILLA, DE FORMA ALEATORIA
	CArena* pc_arena=m_pcSimulator->GetArena();
  
  	dVector2 vCenter[m_nLightObjectNumber];
  	for( int i = 0 ; i < m_nLightObjectNumber ; i++)
  	{
	...
	int nCounter1 = 0;
  	...
  	while(it_lightobject!=vLightObject.end())
  	{
  		(*it_lightobject)->SetCenter(vCenter[nCounter1]);
    	nCounter1++;
    	it_lightobject++;
  	}
  
	//PARA GENERAR LA LUZ AZUL, DE FORMA ALEATORIA
  	dVector2 vCenter2[m_nBlueLightObjectNumber];
  	for( int i = 0 ; i < m_nBlueLightObjectNumber ; i++)
  	{
	...
	int nCounter2 = 0;
  	...
  	while(it_bluelightobject!=vBlueLightObject.end())
  	{
    	(*it_bluelightobject)->SetCenter(vCenter2[nCounter2]);
    	nCounter2++;
    	it_bluelightobject++;
  	}

	//PARA GENERAR LA LUZ AZUL, DE FORMA ALEATORIA
  	dVector2 vCenter3[m_nRedLightObjectNumber];
  	for( int i = 0 ; i < m_nRedLightObjectNumber ; i++)
  	{
    	vCenter3[i].x = ( Random::nextDouble() * m_fInitAreaX * 2.0 ) - m_fInitAreaX;
    	vCenter3[i].y = ( Random::nextDouble() * m_fInitAreaY * 2.0 ) - m_fInitAreaY;
  	}
	...
	int nCounter3 = 0;
  	...
  	while(it_redlightobject!=vRedLightObject.end())
  	{
    	(*it_redlightobject)->SetCenter(vCenter3[nCounter3]);
    	nCounter3++;
    	it_redlightobject++;
  	}
